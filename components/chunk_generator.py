...
class ChunkGenerator:
    ...
    def _validate_transcripts(self, transcripts: List[Dict[str, Any]]):
        ...
        for i, t in enumerate(transcripts):
            if not isinstance(t, dict):
                raise TypeError(f"Item at index {i} in 'transcripts' must be a dictionary.")
            if "number" not in t or not isinstance(t["number"], int):
                raise ValueError(f"Item at index {i} in 'transcripts' must have an integer key 'number'.")
            if "transcript" not in t or not isinstance(t["transcript"], str):
                raise ValueError(f"Item at index {i} in 'transcripts' must have a string key 'transcript'.")
    ...
    def _get_original_transcript_text(self, transcript_number, original_transcripts):
        original_transcript_obj = next((tr for tr in original_transcripts if tr["number"] == transcript_number), None)
        if original_transcript_obj is None:
            raise ValueError(f"Original transcript with number {transcript_number} not found.")
        return re.sub(r'[^\w\s]', '', original_transcript_obj["transcript"].lower()).strip()

    def _handle_short_transcript(self, transcript, original_transcripts, chunks, chunk_number):
        """Handles transcripts shorter than CHUNK_SIZE."""
        original_transcript = self._get_original_transcript_text(transcript["number"], original_transcripts)
        chunk = self._create_chunk(chunk_number, transcript["number"], "normal", original_transcript)
        chunks.append(chunk)
        return chunk_number + 1

    def _generate_normal_and_last_chunks(self, words, transcript_number, current_transcript_idx, total_transcripts, chunks, chunk_number):
        ...
        for i in range(n - CHUNK_SIZE + 1):
            ...
            chunk = self._create_chunk(chunk_number, transcript_number, chunk_type, " ".join(chunk_words))
            chunks.append(chunk)
            chunk_number += 1
        return chunk_number
    ...
    def _generate_hybrid_chunks(self, prev_words, next_words, chunks, chunk_number):
        ...
        if prev_words and next_words:
            for k in range(CHUNK_SIZE-1, 0, -1):
                ...
                chunk = self._create_chunk(chunk_number, -1, "hybrid", " ".join(chunk_words))
                chunks.append(chunk)
                chunk_number += 1
        return chunk_number

    def _preprocess_transcripts_parallel(self, transcripts: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Preprocesses transcripts in parallel."""
        cleaned_transcripts = []
        logging.info("Starting parallel preprocessing of transcripts.")
        start_time = time.perf_counter()
        with concurrent.futures.ThreadPoolExecutor() as executor:
            futures = {executor.submit(self.clean_and_split, t["transcript"]): t["number"] for t in transcripts}
            for future in concurrent.futures.as_completed(futures):
                ...
                try:
                    ...
                    logging.error(f'Transcript {transcript_number} generated an exception: {exc}')
        ...
        logging.info(f"It took {end_time - start_time} seconds to preprocess transcripts.")
        return cleaned_transcripts

    def _process_transcript(self, transcript, cleaned_transcripts, original_transcripts, chunks, chunk_number):
        """Processes a single cleaned transcript to generate chunks."""
        words = transcript["words"]
        transcript_number = transcript["number"]
        n = len(words)
        idx = cleaned_transcripts.index(transcript) # Need index for last transcript check
        ...
        if n < CHUNK_SIZE:
            chunk_number = self._handle_short_transcript(transcript, original_transcripts, chunks, chunk_number)
            return chunk_number

        # Handle normal and last chunks for transcripts >= CHUNK_SIZE
        chunk_number = self._generate_normal_and_last_chunks(words, transcript_number, idx, len(cleaned_transcripts), chunks, chunk_number)
        ...
        if idx < len(cleaned_transcripts) - 1:
            ...
            chunk_number = self._generate_hybrid_chunks(prev_words, next_words, chunks, chunk_number)

        return chunk_number

    def generate_chunks(self, transcripts: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for t in cleaned_transcripts:
            chunk_number = self._process_transcript(t, cleaned_transcripts, transcripts, chunks, chunk_number)
...
if __name__ == "__main__":
    ...
    logging.info(f"It took {general_end_time - general_start_time} seconds to make everything.")
