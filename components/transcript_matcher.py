...
class TranscriptMatcher:
    def match_speech(self, speech: str, chunks: List[Dict[str, Any]], threshold: float) -> Optional[Result]:
        logging.info(f"Starting speech matching for speech: '{speech[:50]}...' against {len(chunks)} chunks with threshold: {threshold}")
        ...
        chunk_id = chunk["id"]
        logging.info(f"Best match found: Chunk ID {chunk_id}, Similarity: {similarity:.2f}, Type: {chunk_type}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        logging.info(f"{len(chunks)} chunks has been loaded from data/chunks/{speaker_name}.json.")
