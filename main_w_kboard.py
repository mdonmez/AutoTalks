...
try:  # try to load chunks if they exist
    ...
    with open(transcript_file, "r", encoding="utf-8") as f:
        chunk_data = chunk_generator.generate_chunks(
            json.load(f)
        )  # generate chunks from transcripts
...
def process_speech(live_text: str) -> str:
    ...
    p, c, n = (
        [
            x
            for x in chunk_data
            if x["matched_transcript"] == current_transcript_number - 1
        ],
        [x for x in chunk_data if x["matched_transcript"] == current_transcript_number],
        [
            x
            for x in chunk_data
            if x["matched_transcript"] == current_transcript_number + 1
    ...
    partial_chunks += [
        x
        for x in chunk_data
        if x["matched_transcript"] == -1
        and x["type"] == "hybrid"
        and (not p or x["chunk_number"] > p[-1]["chunk_number"])
        and (not c or x["chunk_number"] < c[0]["chunk_number"])
    ]
    partial_chunks += c
    partial_chunks += [
        x
        ...
        if x["matched_transcript"] == -1
        and x["type"] == "hybrid"
        and (not c or x["chunk_number"] > c[-1]["chunk_number"])
        and (not n or x["chunk_number"] < n[0]["chunk_number"])
    ]
    partial_chunks += n

    # primary search that are fast
    matcher_result = transcript_matcher.match_speech(
        speech_for_matching, partial_chunks, primary_threshold
    )
    ...
    #     matcher_result = transcript_matcher.match_speech(
    #         speech_for_matching, chunk_data, secondary_threshold
    #     )
    #     if matcher_result:
    #         new_transcript_number = matcher_result.matched_transcript_number
    #         # current_transcript_number = new_transcript_number
